...
var ObjectTableCatalogItem = require('./ObjectTableCatalogItem');
 ...
 *
 * @alias ObjectCsvCatalogItem
 * @constructor
 * @extends ObjectTableCatalogItem
 *
 ...
 */
var ObjectCsvCatalogItem = function(terria, url, options) {
    ObjectTableCatalogItem.call(this, terria, url, options);
...
inherit(ObjectTableCatalogItem, ObjectCsvCatalogItem);


defineProperties(ObjectCsvCatalogItem.prototype, {
    /**
     * Gets the type of data member represented by this instance.
     * @memberOf ObjectCsvCatalogItem.prototype
     * @type {String}
     */
    type: {
        get: function() {
    ...
    typeName: {
        get: function() {
            return 'Object Comma-Separated Values (CSV)';
        }
    },
...
ObjectCsvCatalogItem.defaultUpdaters = clone(ObjectTableCatalogItem.defaultUpdaters);

ObjectCsvCatalogItem.defaultUpdaters.sourceCatalogItem = function() {
    // TODO: For now, don't update from JSON. Better to do it via an id?
};

freezeObject(ObjectCsvCatalogItem.defaultUpdaters);

ObjectCsvCatalogItem.defaultSerializers = clone(ObjectTableCatalogItem.defaultSerializers);

ObjectCsvCatalogItem.defaultSerializers.sourceCatalogItem = function() {
    // TODO: For now, don't serialize. Can we do it via an id?
};

freezeObject(ObjectCsvCatalogItem.defaultSerializers);

/**
 * Loads the TableStructure from a csv file.
 *
 * @param {ObjectCsvCatalogItem} item Item that tableDataSource is created for
 ...
 */
ObjectCsvCatalogItem.prototype.loadIntoTableStructure = function(url) {
    const item = this;
 ...
 */
ObjectCsvCatalogItem.prototype.startPolling = function() {
    const polling = this.polling;
    //Comes second
    //Clustering
    var colorArray = this.dataSource._legendHelper._noColumnColorArray;
    var entityColor = Color.fromBytes(colorArray[0],colorArray[1],colorArray[2],colorArray[3]);
    this.dataSource.clustering.enabled = true;
    this.dataSource.clustering.clusterPoints = true;
    this.dataSource.clustering.pixelRange = 10;
    ...
    this.dataSource.clustering.clusterEvent.addEventListener(function(entities, cluster) {
        ...
        cluster.point.color = entityColor;
        cluster.point.outlineWidth = 2;
        cluster.point.pixelSize = 20;
        //cluster.point.disableDepthDistance = Number.POSITIVE_INFINITY;
        cluster.label.show = true;
        cluster.point.id = cluster.label.id;
        //cluster.style = LabelStyle.FILL_AND_OUTLINE;
        cluster.label.font = 'small-caps bold 20px/2 cursive';
        cluster.label.horizontalOrigin = HorizontalOrigin.CENTER;
        cluster.label.verticalOrigin = VerticalOrigin.CENTER;
        cluster.label.eyeOffset = new Cartesian3(0.0, 0.0, -20.0);
...
ObjectCsvCatalogItem.prototype._load = function() {
    var that = this;
    //Comes first
        ...
        return when(that.data, function(data) {
            if (typeof Blob !== 'undefined' && data instanceof Blob) {
                ...
                throw new TerriaError({
                    ...
                    message: 'ObjectCsvCatalogItem data is expected to be a Blob, File, or String, but it was not any of these. ' +
...
module.exports = ObjectCsvCatalogItem;
