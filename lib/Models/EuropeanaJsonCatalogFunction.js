...
EuropeanaJsonCatalogFunction.prototype.invoke = function() {
    ...
    const promise = loadJson(proxiedUrl).then(json => {
        ...
        query.featureInfoTemplate.template = "<div class='abc'><small>{{Timeline}}</small>{{#Preview}}<figure><img src='{{Preview}}'/><figcaption>{{Description}}</figcaption></figure>{{/Preview}}<p><a target='_blank' href={{SourcePage}}>Original page</a></div>";
