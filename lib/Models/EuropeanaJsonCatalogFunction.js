...
EuropeanaJsonCatalogFunction.prototype.invoke = function() {
    ...
    const promise = loadJson(proxiedUrl).then(json => {
        ...
        let results = json.itemsCount;
        let totalResults = json.totalResults;
        //FIX THIS LATER
        if(totalResults === 0)
        {
        ...
        //let wantedParams = ['dcTitle'];
        let wantedParams = ['Id','Title','Creator','Description','Date','Location','Latitude', 'Longitude', 'Preview', 'Type', 'SourcePage'];
        let itemResults = json.items;
        ...
        itemResults.forEach(function findParams(item, index){
            ...
            let place = item.edmPlaceLabel;
            if(place !== undefined)
                csv_text += place[0].def + ",";
            else
                csv_text += "-,";

            let latitude = item.edmPlaceLatitude;
            if(latitude !== undefined)
                csv_text += latitude[0] + ",";
            else
                csv_text += "NA,";

            let longitude = item.edmPlaceLongitude;
            if(longitude !== undefined)
                csv_text += longitude[0] + ",";
            else
                csv_text += "NA,";

            let thumbnail = item.edmPreview;
        ...
        query.featureInfoTemplate.template = "<div class='cho'><center>{{#Preview}}<figure><img src='{{Preview}}'/><figcaption>{{Description}}</figcaption></figure>{{/Preview}}<p><small>by {{Creator}}, {{Date}}</small><p><a target='_blank' href={{SourcePage}}></a></center></div>";
