 ...
 */
function PersonalJsonCatalogFunction(terria) {
    ...
    console.log("TERRIA",terria);

    knockout.track(this, ['url', 'inputs']);
}

inherit(CatalogFunction, PersonalJsonCatalogFunction);

defineProperties(PersonalJsonCatalogFunction.prototype, {
    /**
    ...
    updaters : {
        get : function() {
            return PersonalJsonCatalogFunction.defaultUpdaters;
        }
    },
    ...
    serializers : {
        get : function() {
            return PersonalJsonCatalogFunction.defaultSerializers;
        }
    },
    ...
    propertiesForSharing : {
        get : function() {
            return PersonalJsonCatalogFunction.defaultPropertiesForSharing;
        }
    },
...
PersonalJsonCatalogFunction.defaultUpdaters = clone(CatalogFunction.defaultUpdaters);

PersonalJsonCatalogFunction.defaultUpdaters.inputs = function(catalogFunction, json, propertyName, options) {
    if (!json.inputs) {
        return;
    }
...
freezeObject(PersonalJsonCatalogFunction.defaultUpdaters);

PersonalJsonCatalogFunction.defaultSerializers = clone(CatalogFunction.defaultSerializers);

PersonalJsonCatalogFunction.defaultSerializers.inputs = function(catalogFunction, json, propertyName, options) {
...
freezeObject(PersonalJsonCatalogFunction.defaultSerializers);

PersonalJsonCatalogFunction.defaultPropertiesForSharing = clone(CatalogFunction.defaultPropertiesForSharing);

PersonalJsonCatalogFunction.prototype._load = function() {
};
 ...
 * @return {Promise}
 */
PersonalJsonCatalogFunction.prototype.invoke = function() {
    var now = new Date();
    ...
    asyncResult.name = 'Submited objects: "' + term + '"';
    else asyncResult.name = 'All submited objects';
    ...
    function searchTerm(results) {
        ...
        {
            // Search in title
            let title = results[result].dcTitle;
            if(defined(title))
            {
            ...
            // Search in creator
            let creator = results[result].dcCreator;
            if(defined(creator))
            {
            ...
            let desc = results[result].dcDescription;
            if(defined(desc))
            {
            ...
            //Search in location
            let location = results[result].locations;
            if(defined(location))
            {
    ...
    const promise = loadJson(proxiedUrl, headers).then(json => {
        const pobj = this.terria.personalObjects;
        ...
        let itemResultsPromise = pobj.results;

        let itemResults;
        if(defined(term)) itemResults = searchTerm(itemResultsPromise); else itemResults = itemResultsPromise;
        ...
        query.featureInfoTemplate.template = "<div class='cho'><center>{{#Preview}}<figure ><img style='max-width: 60%; height:auto;' src='{{Preview}}'/><figcaption>{{Description}}</figcaption></figure>{{/Preview}}<p><small>by {{Creator}}, {{Temporal}}</small></center></div>";
...
module.exports = PersonalJsonCatalogFunction;
