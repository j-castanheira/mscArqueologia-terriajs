...
PersonalJsonCatalogFunction.prototype.invoke = function() {
    ...
    if(defined(term))
    asyncResult.name = 'Submited objects: "' + this.parameters[0].value + '"';
    ...
    const promise = loadJson(proxiedUrl, headers).then(json => {
        ...
        query.featureInfoTemplate.template = "<div class='cho'><center>{{#Preview}}<figure><img src='{{Preview}}'/><figcaption>{{Description}}</figcaption></figure>{{/Preview}}<p><small>by {{Creator}}, {{Temporal}}</small></center></div>";
