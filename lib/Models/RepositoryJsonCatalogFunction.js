...
RepositoryJsonCatalogFunction.prototype.invoke = function() {
    ...
    const promise = loadJson(proxiedUrl, headers).then(json => {
        ...
        query.featureInfoTemplate.template = "<div class='abc'><small>{{Date}}</small>{{#Preview}}<figure><img src='{{Preview}}'/><figcaption>{{Description}}</figcaption></figure>{{/Preview}}<p>{{Repository}}<a target='_blank' href={{SourcePage}}>Original page</a></div>";
