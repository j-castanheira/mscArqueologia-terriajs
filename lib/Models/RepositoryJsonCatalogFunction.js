...
RepositoryJsonCatalogFunction.prototype.invoke = function() {
    ...
    const promise = loadJson(proxiedUrl, headers).then(json => {
        ...
        //Convert response:
        let csv_text = "";
        let wantedParams = ['Id','Title','Creator','Description','Date','Location','Latitude', 'Longitude', 'Preview', 'Repository','SourcePage','SourceData'];
        ...
        itemResults.forEach(function findParams(item, index){
            ...
            if(titles !== undefined)
                csv_text += "\"" + removeDelimiters(titles[0].text[0]) + "\",";
            else
                csv_text += "-,";
