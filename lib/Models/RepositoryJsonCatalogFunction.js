 ...
 */
function RepositoryJsonCatalogFunction(terria) {
...
inherit(CatalogFunction, RepositoryJsonCatalogFunction);

defineProperties(RepositoryJsonCatalogFunction.prototype, {
    /**
    ...
    updaters : {
        get : function() {
            return RepositoryJsonCatalogFunction.defaultUpdaters;
        }
    },
    ...
    serializers : {
        get : function() {
            return RepositoryJsonCatalogFunction.defaultSerializers;
        }
    },
    ...
    propertiesForSharing : {
        get : function() {
            return RepositoryJsonCatalogFunction.defaultPropertiesForSharing;
        }
    },
...
RepositoryJsonCatalogFunction.defaultUpdaters = clone(CatalogFunction.defaultUpdaters);

RepositoryJsonCatalogFunction.defaultUpdaters.inputs = function(catalogFunction, json, propertyName, options) {
    if (!json.inputs) {
        return;
    }
...
freezeObject(RepositoryJsonCatalogFunction.defaultUpdaters);

RepositoryJsonCatalogFunction.defaultSerializers = clone(CatalogFunction.defaultSerializers);

RepositoryJsonCatalogFunction.defaultSerializers.inputs = function(catalogFunction, json, propertyName, options) {
...
freezeObject(RepositoryJsonCatalogFunction.defaultSerializers);

RepositoryJsonCatalogFunction.defaultPropertiesForSharing = clone(CatalogFunction.defaultPropertiesForSharing);

RepositoryJsonCatalogFunction.prototype._load = function() {
};
 ...
 * @return {Promise}
 */
RepositoryJsonCatalogFunction.prototype.invoke = function() {
    var now = new Date();
    ...
    asyncResult.name = '"' + this.parameters[0].value + '"';
    ...
    const promise = loadJson(proxiedUrl, headers).then(json => {
        ...
        let results = json.count;
        //console.log("results",results);
        if(results === 0)
        {
            CesiumEvent.raiseEvent(new TerriaError({
                    sender: this,
        ...
        //Convert response:
        let csv_text = "";
        let wantedParams = ['Id','Title','Creator','Description','Date','Location','Latitude', 'Longitude', 'Preview', 'Type', 'Repository','SourcePage','SourceData'];
        let itemResults = json.results;
        ...
        query.featureInfoTemplate.template = "<div class='cho'><center>{{#Preview}}<figure><img src='{{Preview}}'/><figcaption>{{Description}}</figcaption></figure>{{/Preview}}<p><small>by {{Creator}}, {{Date}}</small><p><a target='_blank' href={{SourcePage}}><b>{{Repository}} Resource </b></a></center></div>";
...
module.exports = RepositoryJsonCatalogFunction;
