...
Cesium.prototype.getAllAttribution = function() {
    const credits = this.scene.frameState.creditDisplay._currentFrameCredits.lightboxCredits;
    return Object.keys(credits).map(id => credits[id].text);
};
...
Cesium.prototype.pickVectorFeatures = function(screenPosition) {
    ...
    var vectorFeatures = [];
    var picked = this.scene.drillPick(screenPosition); //Returns how many (points, labels,etc...) the pick got
    //EDITED
    for (var i = 0; i < picked.length; ++i) {
        var id = picked[i].id;

        if(Array.isArray(id)) //Feature cluster case
        {
            for (var j = 0; j < id.length; ++j) {
                var featureId = id[j];

                if (featureId instanceof Entity && vectorFeatures.indexOf(featureId) === -1) {
                    var feature = Feature.fromEntityCollectionOrEntity(featureId);
                    vectorFeatures.push(feature);
                }
            }
        }
        else //Single feature case
        {
