...
var ObjectCsvCatalogItem = require('../../lib/Models/ObjectCsvCatalogItem');
...
describe('CsvCatalogItem with lat and lon', function() {
    ...
    beforeEach(function() {
        ...
        csvItem = new ObjectCsvCatalogItem(terria);
    });

    it('has sensible type and typeName', function() {
        expect(csvItem.type).toBe('object-csv');
        expect(csvItem.typeName).toBe('Object Comma-Separated Values (CSV)');
    });

    it('throws if constructed without a Terria instance', function() {
        expect(function() {
            var viewModel = new ObjectCsvCatalogItem(); // eslint-disable-line no-unused-vars
        }).toThrow();
    });
    ...
    it('can be round-tripped with serializeToJson and updateFromJson', function() {
        ...
        var reconstructed = new ObjectCsvCatalogItem(terria);
    ...
    it('scales points to a size ratio of 300% if scaleByValue true and respects scale value', function(done) {
        ...
        return csvItem.load().then(function() {
            ...
            var csvItem2 = new ObjectCsvCatalogItem(terria);
